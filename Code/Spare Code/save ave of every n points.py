...
# NEED TO CHECK ON FAST SD CARD IF ln (loop number) FUNCTION FOR MATHS TO GET CORRECT SAMPLE RATE
# STILL GIVES CORRECT ACTUAL SAMPLE RATE WITH ADDED IF/ELSE  

def readvsr_nave(channel, no_samples, input_samplerate, n_ave):
    # function to read and save as many datapoints as you like into an array
    # from channel 1 or 2
    # this function is with an adjustable sample rate
    
    # modified to save the average of every n values      
    ...
    while counter <= no_samples:
        ...
        if counter%n_ave is 0:   
            nvals_v.append(v)
            nvals_t.append(t)
            meanv = np.mean(nvals_v)
            varray.append(meanv)
            meant = np.mean(nvals_t)
            tarray.append(meant)
...
def readvbothsr_nave(no_samples,input_samplerate,n_ave):
    ...
    while counter <= no_samples:
        ...
        if counter%n_ave is 0:   
            ...
            meanv1 = np.mean(nvals_v1)
            meanv2 = np.mean(nvals_v2)
            varray1.append(meanv1)
            varray2.append(meanv2)
            
            meant1 = np.mean(nvals_t1)
            meant2 = np.mean(nvals_t2)
            tarray1.append(meant1)
            tarray2.append(meant2)
            #narray.append(counter)
...
def continuous_read(channel, no_samples, input_samplerate, n_ave, no_runs):  
   ...
   #no_runs is how many times we run readv().
    ...
    while counter<= no_runs:
        t,v,start,end = readvsr_nave(channel, no_samples, input_samplerate, n_ave)
...
def continuous_readboth(no_samples, input_samplerate, n_ave, no_runs):
    ...
    endtimes2 = []
    while counter <= no_runs:
        t1,t2,v1,v2,s1,s2,e1,e2 = readvbothsr_n(no_samples, input_samplerate, n_ave)
